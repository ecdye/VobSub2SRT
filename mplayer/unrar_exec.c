  ...
  if (access(archive, R_OK)) return NULL;
  {
    ...
    if (pid == 0) {
      ...
      if (action == UNRAR_LIST)
        ...
        execl(executable, executable, "p", "-inul", "-p-", archive, filename,
              NULL);
...
int unrar_exec_get(unsigned char **output, unsigned long *size,
  ...
  rar_pipe =
      launch_pipe(&pid, unrar_executable, UNRAR_EXTRACT, rarfile, filename);
  ...
  *output = malloc(bufsize);
  ...
  while (*output) {
    ...
    *size += bytesread;
    if (*size == bufsize) {
      char *p;
      if (bufsize >= 0x7fffffff - ALLOC_INCR) break;
      bufsize += ALLOC_INCR;
...
int unrar_exec_list(const char *rarfile, ArchiveList_struct **list) {
  ...
  while (fgets(buf, sizeof(buf), rar_pipe)) {
    ...
    if (state == PARSE_PROPS &&
        sscanf(p, "%d %d %d%% %d-%d-%d %d:%d", &unpsize, &packsize, &ratio,
  ...
  fclose(rar_pipe);
  pid = waitpid(pid, &status, 0);
  if (file_num < 0 || (pid == -1 && errno != ECHILD) || (pid > 0 && status)) {
