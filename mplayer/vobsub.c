...
static rar_stream_t *rar_open(const char *const filename,
  ...
  if (stream->file == NULL) {
    ...
    if (!rc) {
       ...
       * the files we are looking for have been given arbitrary names in the
       * archive. Let's look for a file with an exact match in the extension
       * only. */
        ...
        if (demanded_ext) {
          ...
          for (i = 0, lp = list; i < num_files; i++, lp = lp->next) {
            ...
            if (name_len >= demanded_ext_len &&
                !strcasecmp(lp->item.Name + name_len - demanded_ext_len,
                            demanded_ext)) {
              rc = unrar_exec_get(&stream->data, &stream->size, lp->item.Name,
                                  rar_filename);
              if (rc) break;
            }
          }
...
static int rar_getc(rar_stream_t *stream) {
  if (stream->file) return getc(stream->file);
  if (rar_eof(stream)) return EOF;
...
static size_t rar_read(void *ptr, size_t size, size_t nmemb,
  ...
  if (stream->file) return fread(ptr, size, nmemb, stream->file);
  if (rar_eof(stream)) return 0;
  res = size * nmemb;
  remain = stream->size - stream->pos;
  if (res > remain) res = remain / size * size;
...
static mpeg_t *mpeg_open(const char *filename) {
  ...
  if (!err) {
    ...
    err = res->stream == NULL;
    if (err) perror("fopen Vobsub file failed");
    if (err) free(res);
  }
  return err ? NULL : res;
}
...
static void mpeg_free(mpeg_t *mpeg) {
  free(mpeg->packet);
  if (mpeg->stream) rar_close(mpeg->stream);
  free(mpeg);
}

static int mpeg_eof(mpeg_t *mpeg) { return rar_eof(mpeg->stream); }

static uint64_t mpeg_tell(mpeg_t *mpeg) { return rar_tell(mpeg->stream); }
...
static int mpeg_run(mpeg_t *mpeg) {
  ...
  mpeg->aid = -1;
  mpeg->packet_size = 0;
  if (rar_read(buf, 4, 1, mpeg->stream) != 1) return -1;
  ...
  switch (buf[3]) {
    ...
    case 0xba: /* Packet start code */
      ...
      if ((c & 0xc0) == 0x40)
        ...
        mp_msg(MSGT_VOBSUB, MSGL_ERR,
               "VobSub: Unsupported MPEG version: 0x%02x\n", c);
        return -1;
      }
      if (version == 4) {
        if (rar_seek(mpeg->stream, 9, SEEK_CUR)) return -1;
      } else if (version == 2) {
        if (rar_seek(mpeg->stream, 7, SEEK_CUR)) return -1;
      } else
        abort();
      if (!mpeg->padding_was_here) mpeg->merge = 1;
      break;
    case 0xbd: /* packet */
      if (rar_read(buf, 2, 1, mpeg->stream) != 1) return -1;
      ...
      if ((c & 0xC0) == 0x40) { /* skip STD scale & size */
        if (rar_getc(mpeg->stream) < 0) return -1;
        c = rar_getc(mpeg->stream);
        if (c < 0) return -1;
      }
      if ((c & 0xf0) == 0x20) { /* System-1 stream timestamp */
        ...
        if (dataidx > idx + len) {
          mp_msg(MSGT_VOBSUB, MSGL_ERR,
                 "Invalid header length: %d (total length: %d, idx: %d, "
                 "dataidx: %d)\n",
                 hdrlen, len, idx, dataidx);
          return -1;
        }
        if ((pts_flags & 0xc0) == 0x80) {
          if (rar_read(buf, 5, 1, mpeg->stream) != 1) return -1;
          if (!(((buf[0] & 0xf0) == 0x20) && (buf[0] & 1) && (buf[2] & 1) &&
            ...
            mpeg->pts = ((buf[0] & 0x0e) << 29 | buf[1] << 22 |
    ...
    case 0xbe: /* Padding */
      if (rar_read(buf, 2, 1, mpeg->stream) != 1) return -1;
      len = buf[0] << 8 | buf[1];
      if (len > 0 && rar_seek(mpeg->stream, len, SEEK_CUR)) return -1;
...
static void packet_destroy(packet_t *pkt) { free(pkt->data); }
...
/* Make sure there is enough room for needed_size packets in the
   packet queue. */
static int packet_queue_ensure(packet_queue_t *queue,
                               unsigned int needed_size) {
  if (queue->packets_reserve < needed_size) {
    if (queue->packets) {
      packet_t *tmp = realloc(queue->packets,
                              2 * queue->packets_reserve * sizeof(packet_t));
      if (tmp == NULL) {
...
static int packet_queue_grow(packet_queue_t *queue) {
  if (packet_queue_ensure(queue, queue->packets_size + 1) < 0) return -1;
...
static int packet_queue_insert(packet_queue_t *queue) {
  packet_t *pkts;
  if (packet_queue_ensure(queue, queue->packets_size + 1) < 0) return -1;
  /* XXX packet_size does not reflect the real thing here, it will be updated a
   * bit later */
...
static int vobsub_ensure_spu_stream(vobsub_t *vob, unsigned int index) {
  if (index >= vob->spu_streams_size) {
    ...
    if (vob->spu_streams) {
      packet_queue_t *tmp =
          realloc(vob->spu_streams, (index + 1) * sizeof(packet_queue_t));
      if (tmp == NULL) {
        mp_msg(MSGT_VOBSUB, MSGL_ERR,
...
static int vobsub_add_id(vobsub_t *vob, const char *id, size_t idlen,
  ...
  if (vobsub_ensure_spu_stream(vob, index) < 0) return -1;
  if (id && idlen) {
...
static int vobsub_add_timestamp(vobsub_t *vob, uint64_t filepos, int ms) {
  ...
  if (vob->spu_streams == 0) {
    mp_msg(MSGT_VOBSUB, MSGL_WARN,
           "[vobsub] warning, binning some index entries. Check your index "
           "file\n");
    return -1;
  }
...
static int vobsub_parse_timestamp(vobsub_t *vob, const char *line) {
  int h, m, s, ms;
  uint64_t filepos;
  if (sscanf(line, " %02d:%02d:%02d:%03d, filepos: %09" PRIx64 "", &h, &m, &s,
             &ms, &filepos) != 5)
    return -1;
  return vobsub_add_timestamp(vob, filepos,
                              vob->delay + ms + 1000 * (s + 60 * (m + 60 * h)));
}
 ...
 */
static __attribute__((always_inline)) inline __attribute__((const)) uint8_t
...
static int vobsub_set_lang(const char *line) {
  if (vobsub_id == -1) vobsub_id = atoi(line + 8);
  return 0;
}
...
static int vobsub_parse_one_line(vobsub_t *vob, rar_stream_t *fd,
  ...
  do {
    ...
    *extradata_len += line_size;
    ...
    if (res < 0) mp_msg(MSGT_VOBSUB, MSGL_ERR, "ERROR in %s", line);
    break;
  } while (1);
  free(line);
  return res;
...
void *vobsub_open(const char *const name, const char *const ifo,
  ...
  if (spu) *spu = NULL;
  if (vobsubid == -2) vobsubid = vobsub_id;
  if (vob) {
    char *buf;
    ...
    if (buf) {
      ...
      if (!ifo) {
        strcpy(buf, name);
        strcat(buf, ".ifo");
        vobsub_parse_ifo(vob, buf, vob->palette, &vob->orig_frame_width,
                         &vob->orig_frame_height, force, -1, NULL);
      } else
        vobsub_parse_ifo(vob, ifo, vob->palette, &vob->orig_frame_width,
                         &vob->orig_frame_height, force, -1, NULL);
      /* read in the index */
      strcpy(buf, name);
      strcat(buf, ".idx");
      ...
      if (spu)
        *spu = spudec_new_scaled(vob->palette, vob->orig_frame_width,
                                 vob->orig_frame_height, extradata,
                                 extradata_len, y_threshold);
      free(extradata);
              ...
              if (vobsub_ensure_spu_stream(vob, sid) >= 0) {
                ...
                if (queue->current_index < queue->packets_size) {
                  ...
                  if (pkt->pts100 != UINT_MAX) {
                    ...
                    mpg->merge = 0;
                    /* FIXME: should not use mpg_sub internal informations, make
                     * a copy */
                    pkt->data = mpg->packet;
...
int vobsub_get_index_by_id(void *vobhandle, int id) {
  vobsub_t *vob = vobhandle;
  int i, j;
  if (vob == NULL || id < 0 || id >= vob->spu_streams_size) return -1;
  if (id != vobsubid && !vob->spu_streams[id].packets_size) return -1;
  for (i = 0, j = 0; i < id; ++i)
    if (i == vobsubid || vob->spu_streams[i].packets_size > 0) ++j;
  return j;
}
    ...
    for (i = 0; i < vob->spu_streams_size; i++)
      if (vob->spu_streams[i].id)
        if ((strncmp(vob->spu_streams[i].id, lang, 2) == 0)) {
          vobsub_id = i;
          mp_msg(MSGT_VOBSUB, MSGL_INFO,
                 "Selected VOBSUB language: %d language: %s\n", i,
                 vob->spu_streams[i].id);
          return 0;
        }
    lang += 2;
...
void vobsub_seek(void *vobhandle, float pts) {
  ...
  if (vob->spu_streams && 0 <= vobsub_id &&
    ...
    if (vobsub_get_id(vob, vobsub_id) == NULL) return;
...
void vobsub_reset(void *vobhandle) {
  ...
  if (vob->spu_streams) {
    ...
    while (n-- > 0) vob->spu_streams[n].current_index = 0;
  }
}
