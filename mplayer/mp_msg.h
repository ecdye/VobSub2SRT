...
#define MSGL_FIXME \
  1  // for conversions from printf where the appropriate MSGL is not known; set
     // equal to ERR for obtrusiveness
#define MSGT_FIXME \
  0  // for conversions from printf where the appropriate MSGT is not known; set
     // equal to GLOBAL for obtrusiveness
...
#define mp_dbg(mod, lev, ...)             \
  do {                                    \
    if (0) mp_msg(mod, lev, __VA_ARGS__); \
  } while (0)
#endif
#endif /* __GNUC__ */
