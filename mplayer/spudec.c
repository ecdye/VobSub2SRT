...
typedef struct {
  ...
  unsigned int
      packet_size; /* size of the packet once all fragments are assembled */
  ...
  int spu_changed;
  unsigned int forced_subs_only; /* flag: 0=display all subtitle, !0 display
                                    only forced subtitles */
  unsigned int
      is_forced_sub; /* true if current subtitle is a forced subtitle */
...
static inline void spudec_cut_image(spudec_handle_t *this) {
  ...
  //  this->start_row, this->image_size, this->stride);

  if (first_y > 0) {
    memmove(this->image, this->image + this->stride * first_y,
            this->stride * this->height);
    memmove(this->aimage, this->aimage + this->stride * first_y,
            this->stride * this->height);
  }
}
 ...
 */
static void pal2gray_alpha(const uint16_t *pal, const uint8_t *src,
                           int src_stride, uint8_t *dst, uint8_t *dsta,
  ...
  for (y = 0; y < h; y++) {
    ...
    if (skip_stride) {
      ...
      for (; x < dst_stride; x++) *dsta++ = *dst++ = 0;
    src += src_stride;
  }
}

static int apply_palette_crop(spudec_handle_t *this, unsigned crop_x,
                              unsigned crop_y, unsigned crop_w,
                              unsigned crop_h) {
  int i;
  uint8_t *src;
  uint16_t pal[4];
  ...
  for (i = 0; i < 4; ++i) {
    ...
    // extend 4 -> 8 bit
    alpha |= alpha << 4;
    if (this->custom && (this->cuspal[i] >> 31) != 0) alpha = 0;
    color =
        this->custom ? this->cuspal[i] : this->global_palette[this->palette[i]];
    color = (color >> 16) & 0xff;
  ...
  pal2gray_alpha(pal, src, this->pal_width, this->image, this->aimage, stride,
                 crop_w, crop_h, 0);
  this->width = crop_w;
  this->height = crop_h;
  this->stride = stride;
...
int spudec_apply_palette_crop(void *this, uint32_t palette, int sx, int sy,
                              int ex, int ey) {
  spudec_handle_t *spu = this;
  struct palette_crop_cache *c = &spu->palette_crop_cache;
  if (c->valid && c->palette == palette && c->sx == sx && c->sy == sy &&
      c->ex == ex && c->ey == ey)
    return c->result;
  ...
  spu->spu_changed = 1;
  c->result = apply_palette_crop(spu, sx - spu->pal_start_col,
                                 sy - spu->pal_start_row, ex - sx, ey - sy);
  c->palette = palette;
  c->sx = sx;
  c->sy = sy;
  c->ex = ex;
...
static void spudec_process_data(spudec_handle_t *this, packet_t *packet) {
  ...
  if (!spudec_alloc_image(this, packet->stride, packet->height)) return;
  ...
  while (packet->current_nibble[0] < i &&
    ...
    if (rle < 0x04) {
      if (rle == 0) {
        ...
        if (rle < 0x04) rle = (rle << 4) | get_nibble(packet);
      }
      rle = (rle << 4) | get_nibble(packet);
    }
    color = 3 - (rle & 0x3);
    ...
    for (type = this->packet[off++]; type != 0xff; type = this->packet[off++]) {
      ...
      switch (type) {
        ...
        case 0x04:
          ...
          // Note: some DVDs change these values to create a fade-in/fade-out
          // effect We can not handle this, so just keep the highest value
        ...
        case 0x05:
          ...
          stride = (width + 7) &
                   ~7; /* Kludge: draw_alpha needs width multiple of 8 */
          start_row = b >> 12;
          end_row = b & 0xfff;
        ...
        default:
          mp_msg(MSGT_SPUDEC, MSGL_WARN,
                 "spudec: Error determining control type 0x%02x. Skipping %d "
                 "bytes.\n",
                 type, next_off - off);
          goto next_control;
      }
...
void spudec_assemble(void *this, unsigned char *packet, unsigned int len,
                     int pts100) {
  spudec_handle_t *spu = this;
  ...
  if ((spu->packet_offset == spu->packet_size) ||
    ...
    while (x + 4 <= spu->packet_offset) {
      ...
      mp_msg(MSGT_SPUDEC, MSGL_DBG2, "SPUtest: x=%d y=%d off=%d size=%d\n", x,
             y, spu->packet_offset, spu->packet_size);
...
{
  spudec_handle_t *spu = this;
  while (spu->queue_head) spudec_free_packet(spudec_dequeue_packet(spu));
  spu->now_pts = 0;
  spu->end_pts = 0;
  spu->packet_size = spu->packet_offset = 0;
}
...
void spudec_heartbeat(void *this, unsigned int pts100) {
  ...
  while (spu->queue_head != NULL && pts100 >= spu->queue_head->start_pts) {
    ...
    if (packet->is_decoded) {
      ...
      if (spu->auto_palette) compute_palette(spu, packet);
...
static void validate_dimensions(spudec_handle_t *spu, unsigned dxs,
                                unsigned dys) {
  ...
  spu->orig_frame_width =
      FFMAX(spu->orig_frame_width, spu->start_col + spu->width);
  spu->orig_frame_height =
      FFMAX(spu->orig_frame_height, spu->start_row + spu->height);
}
  ...
  if (spu->orig_frame_width == 0 || spu->orig_frame_height == 0 ||
    ...
    switch (spu_alignment) {
      ...
      default: /* -1 */
        ...
        bbox[3] =
            spu->start_row * scaley / 0x100 + spu->height * scaley / 0x100;
        break;
    }
  }
}
/* transform mplayer's alpha value into an opacity value that is linear */
static inline int canon_alpha(int alpha) { return (uint8_t)-alpha; }
...
static void scale_table(unsigned int start_src, unsigned int start_tar,
                        unsigned int end_src, unsigned int end_tar,
                        scale_pixel *table) {
  unsigned int t;
...
static void scale_image(int x, int y, scale_pixel *table_x,
  ...
  spu->scaled_image[scaled] = (color[0] * scale[0] + color[1] * scale[1] +
                               color[2] * scale[2] + color[3] * scale[3]) >>
                              24;
  ...
  if (spu->scaled_aimage[scaled]) {
    ...
    spu->scaled_image[scaled] =
        FFMIN(spu->scaled_image[scaled], spu->scaled_aimage[scaled]);
    // convert to MPlayer-style alpha
    spu->scaled_aimage[scaled] = -spu->scaled_aimage[scaled];
  }
}

void spudec_draw_scaled(void *me, unsigned int dxs, unsigned int dys,
                        void (*draw_alpha)(int x0, int y0, int w, int h,
                                           unsigned char *src,
  ...
  if (spudec_visible(spu)) {
    ...
    validate_dimensions(spu, dxs, dys);
    if (!(spu_aamode & 16) &&
        (spu->orig_frame_width == 0 || spu->orig_frame_height == 0 ||
         (spu->orig_frame_width == dxs && spu->orig_frame_height == dys))) {
      spudec_draw(spu, draw_alpha);
    } else {
      if (spu->scaled_frame_width != dxs ||
          spu->scaled_frame_height != dys) { /* Resizing is needed */
        ...
        if (spu->scaled_image_size < spu->scaled_stride * spu->scaled_height) {
          ...
          spu->scaled_image =
              malloc(2 * spu->scaled_stride * spu->scaled_height);
          if (spu->scaled_image) {
          ...
          switch (spu_aamode & 15) {
            case 4:
              mp_msg(MSGT_SPUDEC, MSGL_FATAL,
                     "Fatal: no swsscalar gaussian aa supported");
              break;
            case 3:
              ...
              scale_table(0, 0, spu->height - 1, spu->scaled_height - 1,
                          table_y);
              ...
              for (y = 0; y < spu->scaled_height; ++y) {
                ...
                for (x = 0; x < spu->scaled_width; ++x) {
                  ...
                  for (walky = unscaled_top; walky <= unscaled_bottom; ++walky)
                    for (walkx = unscaled_left; walkx <= unscaled_right;
                         ++walkx) {
                  ...
                  /* spu->scaled_aimage[base] =
                    alpha * dxs * dys / spu->orig_frame_width /
                    spu->orig_frame_height; */
                  if (spu->scaled_aimage[base]) {
                    spu->scaled_aimage[base] = 256 - spu->scaled_aimage[base];
                    if (spu->scaled_aimage[base] + spu->scaled_image[base] >
                        255)
              ...
              for (y = 0; y < spu->scaled_height; ++y) {
                ...
                for (x = 0; x < spu->scaled_width; ++x) {
                  ...
                  const unsigned int width =
                      unscaled_x_right > left_right_column
                  ...
                  /* Now use these informations to compute a good alpha,
                    ...
                    transformed color = sum(surface * alpha * color) /
                    sum(surface * alpha)
                  */
                  /* 1: top left part */
                  ...
                  if (width > 0) {
                    ...
                    for (walkx = left_right_column;
                         walkx < (unsigned int)unscaled_x_right; ++walkx) {
                  ...
                  if (height > 0) {
                    ...
                    for (walky = top_low_row;
                         walky < (unsigned int)unscaled_y_bottom; ++walky) {
                  ...
                  if (width > 0 && height > 0) {
                    ...
                    for (walky = top_low_row;
                      ...
                      for (walkx = left_right_column;
                           walkx < (unsigned int)unscaled_x_right; ++walkx) {
                        tmp = /* 1.0 * 1.0 * */ canon_alpha(
                            spu->aimage[base + walkx]);
                        alpha += tmp;
                  ...
                  if (right > 0.0 && height > 0) {
                    ...
                    for (walky = top_low_row;
                         walky < (unsigned int)unscaled_y_bottom; ++walky) {
                      base =
                          spu->stride * walky + (unsigned int)unscaled_x_right;
                  ...
                  /* Finally mix these transparency and brightness information
                   * suitably */
      ...
      if (spu->scaled_image) {
        switch (spu_alignment) {
          ...
          case 1:
            spu->scaled_start_row =
                dys * sub_pos / 100 - spu->scaled_height / 2;
            if (sub_pos >= 50 &&
                spu->scaled_start_row + spu->scaled_height > dys)
              spu->scaled_start_row = dys - spu->scaled_height;
            break;
          case 2:
        ...
        draw_alpha(spu->scaled_start_col, spu->scaled_start_row,
                   spu->scaled_width, spu->scaled_height, spu->scaled_image,
                   spu->scaled_aimage, spu->scaled_stride);
        spu->spu_changed = 0;
      }
    }
  } else {
...
static void spudec_parse_extradata(spudec_handle_t *this, uint8_t *extradata,
                                   int extradata_len) {
  uint8_t *buffer, *ptr;
  ...
  if (extradata_len == 16 * 4) {
    for (i = 0; i < 16; i++) pal[i] = AV_RB32(extradata + i * 4);
    this->auto_palette = 0;
    return;
  }

  if (!(ptr = buffer = malloc(extradata_len + 1))) return;
  ...
  do {
    ...
    if (!strncmp(ptr, "size: ", 6))
      sscanf(ptr + 6, "%dx%d", &this->orig_frame_width,
    ...
    if (!strncmp(ptr, "palette: ", 9) &&
        sscanf(ptr + 9,
               ...
               &pal[7], &pal[8], &pal[9], &pal[10], &pal[11], &pal[12],
               &pal[13], &pal[14], &pal[15]) == 16) {
      for (i = 0; i < 16; i++) pal[i] = vobsub_palette_to_yuv(pal[i]);
      this->auto_palette = 0;
    }
    if (!strncasecmp(ptr, "forced subs: on", 15)) this->forced_subs_only = 1;
    if (!strncmp(ptr, "custom colors: ON, tridx: ", 26) &&
        sscanf(ptr + 26, "%x, colors: %x, %x, %x, %x", &tridx, cuspal + 0,
               cuspal + 1, cuspal + 2, cuspal + 3) == 5) {
      for (i = 0; i < 4; i++) {
        cuspal[i] = vobsub_rgb_to_yuv(cuspal[i]);
        if (tridx & (1 << (12 - 4 * i))) cuspal[i] |= 1 << 31;
      }
      this->custom = 1;
    }
 ...
 */
static int spudec_yuv_under_threshold(unsigned int yuv,
                                      unsigned int y_threshold) {
...
void *spudec_new_scaled(unsigned int *palette, unsigned int frame_width,
  ...
  if (this) {
    ...
    if (palette)
      ...
      this->auto_palette = 1;
    if (extradata) spudec_parse_extradata(this, extradata, extradata_len);
  ...
  /* Text with overlapping softer edges can reduce the efficacy of the OCR.
     Replace any palette values below threshold with black to create distinct
           characters. */
  for (int i = 0; i < 16; ++i) {
...
packet_t *spudec_packet_create(int x, int y, int w, int h) {
  packet_t *packet;
  int stride = (w + 7) & ~7;
  if ((unsigned)w >= 0x8000 || (unsigned)h > 0x4000) return NULL;
...
void spudec_packet_fill(packet_t *packet, const uint8_t *pal_img,
  ...
  for (i = 0; i < 256; i++) {
    ...
    int alpha = pixel >> 24;
    int gray = (((pixel & 0x000000ff) >> 0) + ((pixel & 0x0000ff00) >> 7) +
                ((pixel & 0x00ff0000) >> 16)) >>
               2;
    gray = FFMIN(gray, alpha);
    g8a8_pal[i] = (-alpha << 8) | gray;
  }
  pal2gray_alpha(g8a8_pal, pal_img, pal_stride, img, aimg, packet->stride, w, h,
                 1);
}

void spudec_packet_send(void *spu, packet_t *packet, double pts,
                        double endpts) {
  packet->start_pts = 0;
  ...
  if (pts != MP_NOPTS_VALUE) packet->start_pts = pts * 90000;
  if (endpts != MP_NOPTS_VALUE) packet->end_pts = endpts * 90000;
...
void spudec_get_data(void *this, const unsigned char **image,
