...
TessBaseAPI* init_tesseract(std::string tesseract_data_path, char const *tess_lang, std::string blacklist, int tesseract_oem, int dpi) {
...
int main(int argc, char **argv) {
  ...
  while( (len = vobsub_get_next_packet(vob, &packet, &timestamp)) > 0) {
    if(timestamp >= 0) {
      ...
      // While tesseract version 3.05 (and older) handle inverted image (dark background and light text) without problem
      ...
      if (threads.size() < static_cast<unsigned>(max_threads)) {
        TessBaseAPI *tess_base_api = init_tesseract(tesseract_data_path, tess_lang, blacklist, tesseract_oem, dpi);
